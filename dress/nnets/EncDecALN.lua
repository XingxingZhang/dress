...
local EncDecA = torch.class('EncDecALN', 'BModel')
...
function EncDecA:createLSTM(x_t, c_tm1, h_tm1, nin, nhid)
  ...
  -- apply layer normalization here
  local ln = nn.LayerNormalization(nhid)
  self:print('apply layer normalization here')
  
  -- compute new hidden state
  local h_t = nn.CMulTable()({ o_t, nn.Tanh()( ln(c_t) ) })
