...
local EncDecA = torch.class('EncDecARF_Plus', 'BModel')

function EncDecA:__init(opts)
  self.opts = opts
  self.name = 'EncDecARF_Plus'
...
function EncDecA:loadLMScorer(lm_path)
   self.lm_scorer = LMScorer(lm_path, self.opts.batchSize)
end


function EncDecA:loadSimScorer(sim_path)
  self.sim_scorer = SimilarityScorer(sim_path, self.opts.batchSize)
end
...
function EncDecA:trainBatch(x, x_mask, y, sgdParam)
  ...
  local function feval(params_)
    ...
    if self.sampleStart <= Ty then
      ...
      -- 1. add SARI as a reward
      local _, _, ori_sents = SARI.getDynBatch(x, x_mask, y, y_rf, self.reward, self.reward_mask, 
        self.opts.src_vocab, self.opts.dst_vocab, self.opts.sariRevWeight)
      -- 2. add LM as a reward ( P(w_1) * P(w_2) * ... * P(w_n) ) ^ (1/n)
      local norm_probs = self.lm_scorer:score(ori_sents.dst)
      -- 3. add Similarity as a reward cos(repr_1, repr_2) and repr_1 and repr_2 are learned by a sequence auto-encoder
      if self.src_repr == nil then
        self.src_repr = self:transData( torch.Tensor(self.opts.batchSize, self.opts.nhid) )
        self.dst_repr = self:transData( torch.Tensor(self.opts.batchSize, self.opts.nhid) )
      end
      local sim_scores = self.sim_scorer:score(ori_sents.src, ori_sents.dst, self.src_repr, self.dst_repr)
      
      -- combine all rewards together
      for i = 1, self.reward:size(2) do
        for j = self.reward:size(1), 1, -1 do
          if self.reward[{ j, i }] ~= 0 then
            self.reward[{ j, i }] = self.reward[{ j, i }] * self.opts.sariWeight 
                + norm_probs[i] * self.opts.lmWeight 
                + sim_scores[i] * self.opts.simWeight
            break
          end
        end
      end
...
function EncDecA:validBatchSample(x, x_mask, y, greedy)
  ...
  local reward_sari = self.reward:sum()
  local reward_lm = torch.Tensor(norm_probs):sum()
  local reward_sim = torch.Tensor(sim_scores):sum()
  ...
  return self.reward:sum(), size, reward_sari, reward_lm, reward_sim
end
