...
function BModel:loadSub(modelPath)
  local md = torch.load(modelPath)
  local sub_size = md:size(1)
  local tot_size = self.params:size(1)
  print('load model size = ' .. sub_size)
  print('model size = ' .. tot_size)
  assert(sub_size < tot_size)
  self.params[{ {1, sub_size} }] = md
  
  xprintln('load sub model from %s done!', modelPath)
end
