...
local NeuLexTrans = torch.class('NeuLexTrans', 'BModel')

function NeuLexTrans:__init(opts)
  self.opts = opts
  self.name = 'NeuLexTrans'
...
function NeuLexTrans:createSoftmax(opts)
  ...
  local model = nn.gModule({h}, {y_prob})
...
function NeuLexTrans:trainBatch(x, x_mask, a, y, sgdParam)
  ...
  local function feval(params_)
    ...
    for t = 1, Tx do
      ...
      local iend = self.opts.batchSize * t
      local istart = iend - self.opts.batchSize + 1
      self.all_enc_hs[{ {istart, iend}, {} }] = self.enc_hs[t][2*self.opts.nlayers]
      
      if self.opts.useGPU then cutorch.synchronize() end
    end
    
    local loss = 0
    local raw_idxs = self:transData( torch.range(0, (self.opts.batchSize - 1) * Tx, Tx):long() )
    local a_hids = {}
    local a_idxs = {}
    local y_preds = {}
    ...
    for t = dec_start, Ty do
      ...
      a_idxs[t] = raw_idxs + a[{ t, {} }]
      a_hids[t] = self.all_enc_hs:index(1, a_idxs[t])
      y_preds[t] = self.dec_softmaxs[t]:forward(a_hids[t])
    ...
    for t = Ty, dec_start, -1 do
      ...
      local df_a_hid = self.dec_softmaxs[t]:backward(a_hids[t], df_y_pred)
    ...
    for t = Tx, 1, -1 do
      local iend = self.opts.batchSize * t
      local istart = iend - self.opts.batchSize + 1
      self.df_dec_h[2*self.opts.nlayers]:add( self.df_all_enc_hs[{ {istart, iend}, {} }] )
      -- apply mask
