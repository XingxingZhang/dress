...
function basename(str)function tds2table(tdsVec)
	local tbl = {}
	for k, v in pairs(tdsVec) do
		if torch.type(v) == 'tds.Vec' or torch.type(v) == 'tds.Hash' then
			tbl[k] = tds2table(v)
		else
			tbl[k] = v
		end
