...
local UniText_Dataset = torch.class('UniText_Dataset')

UniText_Dataset.EOS = '###eos###'  -- for both source and target sequence

function UniText_Dataset.createVocab(inputFile, freqCut, ignoreCase, keepFreq)
  local SENT_END = UniText_Dataset.EOS
...
function UniText_Dataset.sent2ints(vocab, sent)
...
function UniText_Dataset.getDataSize(infiles)
  local sizes = {}
...
function UniText_Dataset.toBatch(sents_src, sents_dst, eos_src, eos_dst, bs)
...
function UniText_Dataset.createBatch(vocab_src, vocab_dst, infile, batchSize, ncedata)
  -- local fin_src = io.open(infile .. '.src')
  -- local fin_dst = io.open(infile .. '.dst')
  local fin_src = io.open(infile)
  local fin_dst = io.open(infile)
  ...
  return function()
    ...
    for i = 1, batchSize do
      ...
      sents_src[#sents_src + 1] = UniText_Dataset.sent2ints(vocab_src, line_src)
      sents_dst[#sents_dst + 1] = UniText_Dataset.sent2ints(vocab_dst, line_dst)
    end
    
    if #sents_src > 0 then
      local x, y = UniText_Dataset.toBatch(sents_src, sents_dst, vocab_src.EOS, vocab_dst.EOS, batchSize)
...
function UniText_Dataset.createBatchShuffle(vocab_src, vocab_dst, infile, batchSize, ncedata)
  -- local fin_src = io.open(infile .. '.src')
  -- local fin_dst = io.open(infile .. '.dst')
  local fin_src = io.open(infile)
  local fin_dst = io.open(infile)
  ...
  while true do
    ...
    table.insert( all_sents_src, UniText_Dataset.sent2ints(vocab_src, line_src) )
    table.insert( all_sents_dst, UniText_Dataset.sent2ints(vocab_dst, line_dst) )
  ...
  return function()
    ...
    if count <= nbatches then
      ...
      for j = rndIdx, rndIdx + batchSize - 1 do
        ...
        sents_src[#sents_src + 1] = all_sents_src[idx] -- UniText_Dataset.sent2ints(vocab_src, all_sents_src[idx])
        sents_dst[#sents_dst + 1] = all_sents_dst[idx] -- UniText_Dataset.sent2ints(vocab_dst, all_sents_dst[idx])
      end
      -- create batches
      local x, y = UniText_Dataset.toBatch(sents_src, sents_dst, vocab_src.EOS, vocab_dst.EOS, batchSize)
      local x_mask = x:ne(0):float()
...
function UniText_Dataset.toBatchX(sents_src, eos_src)
  local function getMaxLen(sents)
    local maxn = 0
    for _, sent in ipairs(sents) do
      if sent:size(1) > maxn then
...
function UniText_Dataset.createBatchX(vocab_src, infile, batchSize)
  local fin_src = io.open(infile .. '.src')
  ...
  return function()
    ...
    if #sents_src > 0 then
      local x = UniText_Dataset.toBatchX(sents_src, vocab_src.EOS)
      local x_mask = x:ne(0):float()
      x[x:eq(0)] = 1
