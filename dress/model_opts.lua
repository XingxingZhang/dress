...
function ModelOpts.getOpts()
  ...
  cmd:option('--recDropout', 0, 'recurent dropout')
  ...
  cmd:option('--encdecPath', '', 'only useful for EncDecARF; this is the pre-trained EncDecA model')
  cmd:option('--sampleStart', 25, 'initial sample start position')
  cmd:option('--deltaSamplePos', 3, 'delta')
  cmd:option('--rfEpoch', 1, 'num of epochs for each rl phrase')
  
  cmd:option('--lmPath', '', '')
  cmd:option('--simPath', '', '')
  cmd:option('--sariWeight', 1, '')
  cmd:option('--lmWeight', 1, '')
  cmd:option('--simWeight', 1, '')
  cmd:option('--sariRevWeight', 0.5, '')
