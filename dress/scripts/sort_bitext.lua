...
local function getOpts()
  ...
  cmd:option('--sort', -1, '0: no sorting of the training data; -1: sort training data by their length; k (k > 0): sort the consecutive k batches by their length')
  cmd:option('--batchSize', 64, 'batch size')
  
  local opts = cmd:parse(arg)
  assert(opts.sort == -1 or opts.sort > 0, 'you must sort!')
  return opts
end
...
local function main()
  ...
  local outLabel = (opts.sort == -1 and '.sort' or string.format('.sort%d', opts.sort))
  -- for training set
  local trainDst = opts.dataset .. '.train.dst'
  local trLens = getLens(trainDst)
  local trIdxs = opts.sort == -1 and Sorter.sortBatches(trLens, opts.batchSize) or Sorter.sortKBatches(trLens, opts.sort * opts.batchSize)
