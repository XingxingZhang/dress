...
local function anonymize(infile)
  ...
  for _, p in ipairs(data) do
    ...
    anonymous_data[#anonymous_data + 1] = tds.Vec({src_sent, dst_sent, tag_id2ne})
  end
  
  -- table.sort(anonymous_data, function(a, b) return #a[2] < #b[2] end)
  -- anonymous_data:sort( function(a, b) return #a[2] < #b[2] end )
...
local function totext(split, infile, label)
  ...
  for _, item in ipairs(split) do
    fout_src:write(item[1]:concat(' ') .. '\n')
    fout_trg:write(item[2]:concat(' ') .. '\n')
    table.insert(map, xtds2table( item[3] ) )
  end
