...
function XSampler:__init(modelPath)
  ...
  -- init model
  local bs = 1
  if opts.model == 'EncDecA' or opts.model == 'EncDecAWE' then bs = 2 end
  self.s = {}
...
function XSampler:generate_att(src_sent)
  ...
  -- encoder
  local Tx = #src_wids
  self.all_enc_hs:resize( 2 * Tx, self.opts.nhid )
  local raw_idxs = self:transData( torch.linspace(0, Tx*(2-1), 2):long() )
  for t = 1, Tx do
    local x_t = self:transData( torch.Tensor({src_wids[t], src_wids[t]}) )
    local nx_s = self.encdec.enc_lstm_master:forward({ x_t, self.s })
    local idxs = raw_idxs + t
    self.all_enc_hs:indexCopy(1, idxs, nx_s[2*self.opts.nlayers])
    model_utils.copy_table(self.s, nx_s)
  end
  local all_enc_hs = self.all_enc_hs:view(2, Tx, self.opts.nhid)
  
  self.h_hat:zero()
  local x_mask_t = self:transData( torch.ones(2, Tx) )
  local x_mask_sub = self:transData( torch.zeros(2, Tx) )
  ...
  while true do
    -- move one step
    local x_t = self:transData( torch.Tensor({prev_word, prev_word}) )
...
function XSampler:generateBatch(dataPath, outPath)
  ...
  while true do
    ...
    elseif self.opts.model == 'EncDecA' or self.opts.model == 'EncDecAWE' then
